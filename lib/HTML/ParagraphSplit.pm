...
This allows authors that want to use HTML markup some but don't really want to cope with getting their paragraph tags right, can use this filter to format their work the right way.
...
This method has three forms, which vary only in the input the receive. If the first argument is a file handle, C<$handle>, then that handle will be read, parsed, and split. If the first argument is a scalar, C<$text>, then that text will parsed and split. If the first argument is a subclass of L<HTML::Element>, C<$element>, then the tree represented by the node will be traversed and split. 
...
If this option is used, then paragrpahs will not be added to your text unless there is at least on double-line break. This option is used internally to make sure nested elements do not have extra C<P>-tags unnecessarily.
...
I've left a few token references to that in the documentation name for kicks.
